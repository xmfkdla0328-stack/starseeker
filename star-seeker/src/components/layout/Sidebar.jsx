...
import { Users, Sparkles, Sprout, Book, Package, Circle } from 'lucide-react';

export const Sidebar = ({ screen, setScreen }) => {
  const NavItem = ({ id, icon: Icon, label }) => (
    <button 
      onClick={() => setScreen(id)}
      className={`w-full p-2 lg:p-3 rounded-xl flex flex-col items-center gap-0.5 lg:gap-1 transition-all duration-300 group relative overflow-hidden
        ${screen === id ? 'text-cyan-200' : 'text-slate-400 hover:text-cyan-200'}
      `}
    >
      {/* 홀로그램 배경 효과 */}
      {screen === id && (
        <>
          <div className="absolute inset-0 bg-gradient-to-b from-cyan-500/20 to-blue-500/10 rounded-xl"></div>
          <div className="absolute inset-0 border border-cyan-400/40 rounded-xl shadow-[0_0_15px_rgba(34,211,238,0.3)]"></div>
      ...
      <div className="relative z-10">
        <Icon size={18} className={`lg:w-[22px] lg:h-[22px] transition-all duration-300 ${screen === id ? 'scale-110 drop-shadow-[0_0_8px_rgba(34,211,238,0.8)]' : 'group-hover:scale-105'}`} />
        {screen === id && (
      ...
      <span className={`text-[9px] font-bold tracking-wider uppercase relative z-10 transition-all ${
        screen === id ? 'text-cyan-50' : 'text-slate-500 group-hover:text-slate-300'
      }`}>{label}</span>
  ...
  return (
    <nav className="w-16 lg:w-20 bg-black/40 backdrop-blur-md border-r border-cyan-500/10 flex flex-col items-center py-3 lg:py-4 gap-2 lg:gap-2 z-50 shrink-0 relative">
      {/* 우주 테마 장식 */}
